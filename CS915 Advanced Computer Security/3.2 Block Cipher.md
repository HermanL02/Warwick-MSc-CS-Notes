...
IV, C1, C2 xor R, C3, modified one of the values
R = {r0, r1, ..., r15}, so we have 15 bytes random, then we try r15 from 0-255 (because each byte is 256.), then we can do some calculation
LB(c) = LB（A) xor LB(B)
LB(B) = 01
LB(p3) = LB(c2) xor LB(c) = r15 xor '01'
